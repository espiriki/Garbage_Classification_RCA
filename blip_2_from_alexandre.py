...
def save_checkpoint(model, optimizer, epoch, step, acc, device):
    filename = "BLIP2_epoch_"+str(epoch)+"_acc_"+str(acc)+".pth"
...
def calculate_acc(model, loader, device, processor, dataset):
    ...
    for idx, b in enumerate(loader):
        with torch.no_grad():
            ...
            # print("prompt in eval: ", processor.tokenizer.batch_decode(batch_dev['input_ids'], skip_special_tokens=True))
            outputs = model.generate(input_ids=batch_dev['input_ids'], pixel_values=batch_dev['pixel_values'], max_new_tokens=1,max_length=1)
            out = [i.replace('<pad>', '').replace('</s>','') for i in processor.tokenizer.batch_decode(outputs, skip_special_tokens=True)]

            # decoded = processor.tokenizer.batch_decode(outputs, skip_special_tokens=True)
            label_decoded = processor.tokenizer.batch_decode(b['labels'], skip_special_tokens=True)

            y_true = [cls_dict[elem.replace('<pad>', '').replace("</s>","")] for elem in processor.tokenizer.batch_decode(b['labels'], skip_special_tokens=True)]
            out_ = [cls_dict[find_closest_string(i, strings)] for i in out]
...
for epoch in range(num_epochs):
    ...
    model.eval()
    train_acc = calculate_acc(model, loader_train, device, processor, "Train")
    val_accuracy = calculate_acc(model, test_loader_val, device, processor, "Validation")
    
    if val_accuracy > max_val_accuracy:
        print("Best model obtained based on Val Acc. Saving it!")
        save_checkpoint(model, optimizer, epoch, step, val_accuracy, device)
