...
def calculate_acc(model, loader, device, processor, dataset):
    ...
    for idx, b in enumerate(loader):
        with torch.no_grad():
            ...
            outs.append(out_)
            gt_labels.append(y_true)
            
            predictions = torch.tensor(outs).view(-1)
            labels = torch.tensor(gt_labels).view(-1)
...
test_accuracy, test_report, test_report_dict, conf_matrix = calculate_acc(model, loader_test_set, device, processor, "Test")
